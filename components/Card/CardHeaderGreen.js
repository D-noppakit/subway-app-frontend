...
export default function CardHeaderGreen({ children, close = () => console.log("ส่ง function ปิดมา"), titleHeader }) {
    const TitleHeader = () => {
        return <div className='h-full text-[18px] text-white mt-2'>{titleHeader}</div>
    }
    return (
            ...
            <div className=" rounded-3xl absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white shadow-lg w-[90%]">
                <div className='h-[70px]rounded-t-3xl relative  '>
                    ...
                    <div className='absolute top-4 left-4 ' >
                        {titleHeader ? <TitleHeader /> : <Image src={LogoWhite} alt='logo' width={35} height={43} style={{ objectFit: "contain" }} />}
                    </div>
