...
export default function Description() {
    ...
    const { AllergiesDescription, setAllergiesDescription } = store();
    useEffect(() => {
        console.log({ AllergiesDescription })
    }, [AllergiesDescription])
    const handleChange = (e) => {
        setAllergiesDescription(e.target.value);
    };
    return (
        <div className='bg-[#00491E] flex flex-col justify-start items-start rounded-2xl p-3 relative mt-3  w-full'>
            ...
            {openInput && <div className='w-full'>
                ...
                <input onChange={handleChange} value={AllergiesDescription} className='bg-white outline-none p-4 w-full rounded-3xl' />
            </div>}
